...
class FileDriver{
  ...
  public function __construct($config)
  {
    ...
    if ( ! is_dir($directory) OR ! is_writable($directory))
      throw new MException('缓存存放目录不可写:'.$directory);
    // Directory is valid
  ...
  public function exists($id, $tag = FALSE)
  {
    ...
    else
    {
      ...
      return glob(rtrim($this->directory,'/').'/'.$id.'~*');
    }
  }
  ...
  public function set($id, $data, array $tags = NULL, $lifetime)
  {
    ...
    return (bool) file_put_contents(rtrim($this->directory,'/').'/'.$id.'~'.$tags.'~'.$lifetime, serialize($data));
  }
  ...
  public function delete($id, $tag = FALSE)
  {
    ...
    foreach ($files as $file)
    {
      ...
      if ( ! unlink($file))
        Fw::log('error', 'Cache: Unable to delete cache file: '.$file);
    }
